...
GetHashTagData(){
  ...
  if(h){
    Axios(`${REQUEST_BASE_URL}/twitter/${this.state.hashTag}/${this.state.resultType}/${this.state.hashTagDataLength || 1}`).then(({data})=>{
      this.setState({
        data:data , 
...
hashTagsUsersClick(t){//@ is removed 
  return new Promise((res) => {
    if (!this.hasUser(t)) {
      Axios(`${REQUEST_BASE_URL}/twitterUser/${t}`).then(({ data }) => {
        let d = {}
        d[t] = data
...
function RenderMedia({ data }){
  ...
  if(!get.img){
    Axios(`${REQUEST_BASE_URL}/getmetadata?url=${link}&types=og:image,og:description`).then(({data})=>{
       let d = data.data
...
class GoogleFeeds extends React.PureComponent{
 ...
 componentDidMount(){
   Axios(`${REQUEST_BASE_URL}/feeds/${this.state.hashTag}`).then(({data})=>{
