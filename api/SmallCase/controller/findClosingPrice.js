...
module.exports = async function findClosingPrice(stockCode, startDate, diffDays, conn){
    ...
    if(nse.length > 0){
        ...
        for(var j=0; j<stockCode.length; j++){
            if(stockCode[j] == 'CASH' || stockCode[j] == 'Cash'){
    ...
    //if there is no nse stockCode
    if(nse.length <= 0){
        closingPriceMatrix[0] = [];
        for(let j=0; j<stockCode.length; j++){
            if(stockCode[j] == 'CASH' || stockCode[j] == 'Cash'){
                closingPriceMatrix[0][j] = 1;
            }else{
                closingPriceMatrix[0][j] = '';
    ...
    //str2="" means there is no bse code
    if(str2=="")    return closingPriceMatrix;
    ...
    // if there is unavailable stockcod then return false status and the unavailable stockcode
    let unavailableStockCode = [];
    for(let i=0; i<stockCode.length; i++){
        if(closingPriceMatrix[0][i]==''){
            unavailableStockCode.push(stockCode[i]);
        }
    }

    if(unavailableStockCode.length>0)
        return {status: false, STOCKCODE: unavailableStockCode};
