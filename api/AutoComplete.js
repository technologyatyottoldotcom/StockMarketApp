...
const AutoComplete = async (query)=>{
    return new Promise((resolve,reject)=>{
        ...
        let sql = `SELECT T1.*,T2.company,T2.code,T2.symbol,T2.exchange,T2.trading_symbol AS filter FROM stock_list_master_reuters_screener_original as T1 
                        JOIN master_security_list_NSE as T2 
                        ON T2.symbol = T1.nse_code AND T2.company LIKE '%${query}%' AND T2.trading_symbol LIKE '%-EQ'
                   UNION
                   SELECT T3.*,T4.company,T4.code,T4.symbol,T4.exchange,T4.sc_group AS filter FROM stock_list_master_reuters_screener_original as T3 
